...
mkdir fig3.t
 for tau in "${tauV[@]}"; do   for i in `seq 100`; do tStart=$(((defaultWidth-defaultStartX-1)*tau)); nice ./sssim $defaultNReplicates -t $defaultTheta --see $defaultWidth $defaultHeight $defaultDemeSize $defaultFounderSize  $defaultMigrationRate $defaultStartX $defaultStartY ${tStart}.001 $tau $ascSample  -o fig3.t/t${tau}_${i} 2>fig3.tau.${tau}_${i}.txt;    done &  done

mkdir fig3.m
for m in "${mRates[@]}"; do   for i in `seq 100`; do nice ./sssim $defaultNReplicates -t $defaultTheta --see $defaultWidth $defaultHeight $defaultDemeSize $defaultFounderSize  $m $defaultStartX $defaultStartY ${defaultTStart}.001 $defaultTau $ascSample  -o fig3.m/m${m}_${i} 2>fig3.m.${m}_${i}.txt;    done &  done
