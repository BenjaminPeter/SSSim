...
void SimulationResults::printStats(){
    int pos=0;
    for(int i=0; i<nSamples-1;i++){
        for(int j=i+1; j<nSamples;j++){
            cerr << i << "\t" << j << "\t" << this->fst[pos] << "\t" << this->psi[pos] << "\t" << this->deltaH[pos] << endl;        
            pos++;
        }
    }
...
void SimulationResults::printStatsWithJK(){
    int pos=0;
    ...
    for(int i=0; i<nSamples-1;i++){
        for(int j=i+1; j<nSamples;j++){
            cerr << i << "\t" << j << "\t";
            cerr << this->fst[pos] << "\t" <<bFST[pos]<<"\t";
            cerr << this->psi[pos] << "\t" <<bPsi[pos]<<"\t";
            cerr << this->deltaH[pos] << "\t" <<bDeltaH[pos]<<"\n";
            pos++;
        }
    }
