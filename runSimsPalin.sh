...
mkdir fig3.k
for k in "${founderSizes[@]}"; do   for i in `seq 100`; do nice ./sssim $defaultNReplicates  --see $defaultWidth $defaultHeight $defaultDemeSize $k  $defaultMigrationRate $defaultStartX $defaultStartY ${defaultTStart}.001 $defaultTau  $ascSample  -o fig3.k/k${k}_${i} 2>fig3.k.${k}_${i}.txt;   done &  done
